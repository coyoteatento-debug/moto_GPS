...
class SmoothLocationService {
  ...
  void _onTick() {
    ...
    if (progress >= 0.95 && _speedMs > 0.5) {
      ...
      const R = 6371000.0;
      final cosLat = cos((_toLat ?? lat) * pi / 180);
      if (cosLat.abs() > 0.001) {
  ...
  Duration _calcDuration(double speedMs) {
    if (speedMs < 1) return const Duration(milliseconds: 1500);
    if (speedMs < 3) return const Duration(milliseconds: 1200);
    if (speedMs < 10) return const Duration(milliseconds: 900);
    if (speedMs < 25) return const Duration(milliseconds: 750);
